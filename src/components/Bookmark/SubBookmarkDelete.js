...
const SubBookmarkDelete = ({ el }) => {
    ...
    const deleteBookmark = (e, bookmark_id) => {
        ...
        clickFetchFunc({
            method: 'DELETE',
            url: `/subBookmark/delete/${bookmark_id}`,
        })
        forceUpdate()
    }
    return (
        <>
            <button className='deleteBtn'><img key={el.id} onClick={(e) => { deleteBookmark(e, el.id); }} src={deleteImg} /></button>
        </>
    );
};

export default SubBookmarkDelete;